...
def exchange_main():
    ...
    #딕셔너리로 통화정보를 만들기
    currency_name_dict={'미국달러':'USD','유럽연합 유로':'EUR','일본엔':'JPY'}
    ...
    if clicked:
        ...
        last_page=10
        df_exchange= get_exchange_rate_data(currency_name_dict[currency_name],last_page)
        ...
        df_exchange_rate= df_exchange_rate.set_index(['날짜']) #1열에 있는 No. 없애고 날짜를 1열에 보내기


        #환율데이터 표시
        st.dataframe(df_exchange_rate)

        #차트그리기(선그래프, 판다스 이용) index가 x축이 됨
        df_exchange_rate_2=df_exchange_rate.copy()
        df_exchange_rate_2.index= pd.to_datetime(df_exchange_rate_2.index)

        ax = df_exchange_rate_2['매매기준율'].plot(grid= True,figsize=(15,5)) #그래프 사이즈 inch
        ...
        col = st.columns(2)
        with col[0]:
            st.download_button('scv 파일 다운로드',csv_data,file_name='exchange_rate_data.csv')

        with col[1]:
            st.download_button('엑셀 파일 다운로드',excel_data,file_name='exchang_rate_data.xlsx')
